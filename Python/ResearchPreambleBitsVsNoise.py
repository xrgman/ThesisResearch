...
PREAMBLE_BITS = 512#4096 #round(T_preamble * sample_rate)
...
preamble_bits_values = [256, 512, 1024, 2048, 4096, 8192, 16384, 32768]
snr_hop_size = 2
max_snr_val = 26
...
results = np.empty((preamble_bit_cycles, nr_of_snr_cycles))
...
for snr_hop in range(nr_of_snr_cycles):
    ...
    for preamble_bits_idx in range(preamble_bit_cycles):
        ...
        results[preamble_bits_idx, snr_hop] = percentage_preamble_correct

        correct_preambles_detected = 0

#print("Successfull runs: " + str(decoding_cycles_success) + ", successfull preambles: " + str(percentage_preamble_correct) + "%")

x_values = [x * -2 for x in range(0, nr_of_snr_cycles)]

#x_positions = np.arange(len(x_values))
bar_width = 0.1
total_group_width = bar_width * preamble_bit_cycles
...
for i, group_data in enumerate(results):
    plt.bar(x_positions + i * bar_width, group_data, width=bar_width, label=f'{preamble_bits_values[i]} samples')
...
plt.title("Correct preamble detection noise vs number of samples")
plt.legend()

plt.savefig('Figures/Encoding/PreambleDetection.png', format='png')
