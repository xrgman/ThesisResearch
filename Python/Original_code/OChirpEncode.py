...
class OChirpEncode:
    ...
    def convert_bit_to_chrirp(self, symbols: list, bit: int, M: int = None, T: float = None, no_window: bool = False,
        ...
        if minimal_sub_chirp_duration is False:
            ...
            for i in range(0, int(np.ceil(Tb * self.fsample))):
                t.append(i / self.fsample)
        ...
        def encode(symbol, Tb, t):
            ...
            for subchirp in symbol:
                ...
                chirpThing = chirp(t, subchirp[0], Tb, subchirp[1])
                subchirp_signal = window_kaiser * self.volume * chirpThing
