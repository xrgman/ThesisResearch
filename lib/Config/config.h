...
public:
    Config() : robotId(-1), totalNumberRobots(-1), sampleRate(-1), numChannelsRaw(-1), numChannels(-1), filterOwnSource(false), printBitsEncoding(false), channels(nullptr),
               ...
               preambleUndersamplingDivisor(-1), kaiserWindowBeta(-1), calibrateSignalEnergyTarget(-1), calibrateSignalEnergy(false), testAllRobots(false), cellSize(-1) {};
    Config(int robotId, int totalNumberRobots, int sampleRate, int numChannelsRaw, int numChannels, bool filterOwnSource, bool printBitsEncoding,
           ...
           double frequencyStopBit, double bandwidthPadding, double calibrateSignalEnergyTarget, bool calibrateSignalEnergy, bool testAllRobots, int cellSize)
        : robotId(robotId), totalNumberRobots(totalNumberRobots), sampleRate(sampleRate), numChannelsRaw(numChannelsRaw),
          ...
          bandwidthPadding(bandwidthPadding), calibrateSignalEnergyTarget(calibrateSignalEnergyTarget), calibrateSignalEnergy(calibrateSignalEnergy), testAllRobots(testAllRobots), cellSize(cellSize)
    {
